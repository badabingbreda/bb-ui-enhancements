...
(function($) {
    ...
    function uiEnhancements() {
      if ( $('body' ).hasClass( 'bb-ui-enhancements' ) ) {
        localStorage.removeItem( 'bb-ui-enhancements' );
      } else {
        localStorage.setItem( 'bb-ui-enhancements', true );
      }
      toggleEnhancementClass();
    }
    
    function toggleEnhancementClass() {
      $( 'body' ).toggleClass('bb-ui-enhancements');
      // check if UIFrame is enabled, otherwise we will call twice and thus toggle back
      if ( FLBuilder?.UIIFrame?.isEnabled() ) {
        $('body' , parent.window.document ).toggleClass('bb-ui-enhancements');
      }
    }

    function startEnhanceHover() {
      $('body').addClass('bb-ui-enhancements-hover');
    }

    function endEnhanceHover() {
      $('body').removeClass('bb-ui-enhancements-hover');
    }
    ...
    function addBeaverXray() {
      ...
      const barTitleIcon = parent.window.document.querySelector( '.fl-builder-bar-title-icon' );
    ...
    function addEnhanceButton() {
      ...
      const builderBar = parent.window.document.querySelector( '.fl-builder-bar-actions' );
      const builderNotifications = parent.window.document.querySelector('#fl-builder-toggle-notifications');
      ...
      // register listener for the fl-builder-bar
      $( '.fl-builder-ui-enhancements' , parent.window.document ).on( 'click' , uiEnhancements );
      $( '.fl-builder-bar' , parent.window.document ).hover( startEnhanceHover, endEnhanceHover );
      

    }

    // Add Enhance Button if notifications toggle exists
    function tryAddingEnhanceButton() {
      // test if this element has already been created by builder
      if ( parent.window.document.querySelector('#fl-builder-toggle-notifications') !== null ) {
        if (bbuie.enhanceButton ) addEnhanceButton();
        if (bbuie.xray ) addBeaverXray();
        return;
      } else {
        setTimeout( tryAddingEnhanceButton , 1000 );
      }
    }

    $(document).ready( function() {
      if ( localStorage.getItem( 'bb-ui-enhancements' )) toggleEnhancementClass();
      // use FLBuilder addHook to add Enhance Button after UI is done creating
      FLBuilder.addHook( 'didInitUI' , tryAddingEnhanceButton );
